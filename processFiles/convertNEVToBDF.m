function convertNEVToBDF(baseDirectory,convertFolders,varargin)
% CONVERTNEVTOBDF Converts all neural data files in a folder to BDF struct
%
%   This script will automatically load neural data and convert it.
% Works with Cerebus (.nev). You select a base
...
%
% NOTES:
%   This program will create a folder called "BDFStructs" inside each of the
% folders directory and put the .mat files for each convertFolders there.
%
...
ignorejumps = 1; %By default: Ignore jumps
...
for iFolder = 1:length(convertFolders)
    ...
    dirBDF = fullfile(baseDirectory, convertFolders{iFolder},filesep, bdfFolderName, filesep); %fullfile(baseDirectory,[filesep bdfFolderName filesep],convertFolders{iFolder},filesep);
        ...
        if ~strcmpi(CB_FullFileName(end-12:end),'-metatags.mat') && (~exist(BDF_FullFileName,'file') || rewriteFiles)
            ...
            switch lower(fileExt)
                case '.nev'
                    ...
                    if ignorejumps
                        out_struct = get_nev_mat_data(CB_FullFileName,'verbose','rothandle',0,1,'ignore_jumps');
                    else
